...
results = load_results('./results/006_MDP.pkl')
...
#CVar_space = results['mpc']['_tvp','cvar_space']

xg = results['mpc']['_tvp','xg',0]
yg = results['mpc']['_tvp','yg',0]
#
# print(xg)
pxx= px+x
pyy= py+y
...
data =  np.vstack((pxx.T, pyy.T, heading.T)).T
print(data.shape)         
with open('MDP.csv', 'w') as f:
    for j in data:
        f.write('%f,%f,%f\n' % tuple(j))
...
#xax= np.arange(0, 48, 1)
#yax = np.arange(0, 40, 1)
#print(CVar_space.shape)
#colormesh = ax.pcolormesh(xax,yax,CVar_space, alpha=0.3)
...
goal = Circle((xg[0], yg[0]), 0.1, color="green", label="goal")
ter_goal = Circle((7.5, 3.5), 0.2, color="red", label="goal")
...
goal_disc = Rectangle((7,3), 1, 1, fc = "green", ec="black" )
#ax.add_patch(wall)
#ax.add_patch(wall2)
#ax.add_patch(goal_disc)
#ax.add_artist(goal)
ax.add_artist(ter_goal)
...
def init():
    ax.add_patch(patchp)
    return patchp, patch, patchp2, patch2, goal
...
def animate(i): 
    ...
    return patchp, patch, patchp2, patch2, patch_com, goal
...
ax.title.set_text('With CVaR MDP')
